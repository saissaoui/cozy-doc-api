...
func (s *Suite) TestpsertDocs_HappyPath() {
	...
	s.store.On("InsertDocument", mock.AnythingOfType("json.RawMessage"), req.Database).Return(nil)
...
func (s *Suite) TestpsertDocs_InsertDocError() {
	...
	s.store.On("CreateDatabase", req.Database).Return(nil)
	s.store.On("InsertDocument", mock.AnythingOfType("json.RawMessage"), req.Database).Return(e)
